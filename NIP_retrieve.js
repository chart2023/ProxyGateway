...
function handleContent(nscl, container, content) {
	//data_container=parseB64Json(content);
	...
	console.log(point, parseB64Json(content).pir.value);
	export_write.write(pointset, point, parseB64Json(content).pir.value);
	console.log("Handled content");
}

function handleContentInstance(nscl, container, contentInstance) {
	dIaClient.requestIndication('RETRIEVE', null, nscl.link + '/applications/' + targetApplicationID + '/containers/' + container.id + '/contentInstances/'+contentInstance).on('STATUS_OK',
		 function(data) {
			handleContent(nscl, container, data.contentInstance.content.$t);
		});
}
...
function handleContainer(nscl, container) {
	...
	/*dIaClient.requestIndication('CREATE', null, nscl.link + '/applications/' + targetApplicationID + '/containers/' + container.id + '/contentInstances/subscriptions', { 
		subscription: {
			contact: notifyUri, filterCriteria: {attributeAccessor: 'latest'}//
		}
	...
	/*dIaClient.requestIndication('RETRIEVE', null, nscl.link + '/applications/' + targetApplicationID + '/containers/' + container.id + '/contentInstances'+'/latest.id').on('STATUS_OK',
		 function(data) {
			handleContentInstance(nscl, container, data.contentInstance);
		});*/
	dIaClient.requestIndication('RETRIEVE', null, nscl.link + '/applications/' + targetApplicationID + '/containers/' + container.id + '/contentInstances').on('STATUS_OK',
		 function(data) {
			//console.log(nscl, container, data.contentInstances.latest.id);
			handleContentInstance(nscl, container, data.contentInstances.latest.id);
			//handleContent(nscl, container, data.contentInstances);
		});
}
...
function initNSCL(nscl) {
	...
	app.post(notifyPath, function(req, res) {
		var representation = getNotificationData(req);
		handleContainers(nscl, representation);
		res.send(200);
	});
...
function handleNSCL(nscl) {
	...
	app.post(notifyPath, function(req, res) {
		...
		handleApplications(nscl, representation);
		res.send(200);
	});
...
function handleNSCLs(nscls) {
	nscls.sclCollection.namedReference.forEach(function(nscl) {
		handleNSCL(nscl);
	});
}
var nscls = {};
