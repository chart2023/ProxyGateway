...
function handleContent(sclId, container, content) {
	console.log(container.id, content.data.consumed);
...
function handleContainer(sclId, appLink, container) {
	...
	app.post(notifyPath, function(req, res) {
	 	fs.appendFile(__dirname + "/notify_nip_gscl", data.point.id +new Date()+"   "+new Date().getMilliseconds()+'\n' , function (err) {
		  if (err) throw err;
		  console.log('The "data to append" was appended to file!: data_gip');

		});
		var representation = getNotificationData(req);
		handleContent(sclId, container, representation);
	 	fs.appendFile(__dirname + "/notifyok_nip_gscl", data.point.id +new Date()+"   "+new Date().getMilliseconds()+'\n' , function (err) {
		  if (err) throw err;
		  console.log('The "data to append" was appended to file!: data_gip');
